...
foreach ($a_files as $k){
	...
	//把线上访问的地址中的版本号替换成本地开发的src目录
	$k = preg_replace('/\d\.\d\.\d/', 'src', $k, 1);
...
foreach ($a_files as $k) {
	...
	//把线上访问的地址中的版本号替换成本地开发的src目录
	$local = preg_replace('/\d\.\d\.\d/', 'src', $k, 1);
	...
	//文件存在
	if($type == "css" && file_exists($lessfile)){
		$R_files[] = $less->compileFile($lessfile);
	} else if (file_exists($local)){
		$R_files[] = file_get_contents($local);
	} else {
		//文件不存在
		try{
			//php4不支持try catch，如果基于php4的话，删掉try catch语句
			$R_files[] = '/* combined from cdn: http://g.tbcdn.cn/'.$k.' */';
			$tfile = file($CDN.$k);
			if(!$tfile){
				$unfound[] = 'http://g.tbcdn.cn/'.$k;
			}
			$R_files[] = join('',$tfile);
